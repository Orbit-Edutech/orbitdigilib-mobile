...
class SewaDigitalCard extends StatelessWidget {
  const SewaDigitalCard({super.key, required this.item});
  final ModelSewa item;

  @override
  Widget build(BuildContext context) {
    final buku = item.bukuAnggota;
    final tanggalHabis = item.tanggalHabisSewa;
    final now = DateTime.now();

    final Color statusColor;
    final String statusLabel;
    if (tanggalHabis != null && now.isAfter(tanggalHabis)) {
      statusColor = AppColor.grey;
      statusLabel = "Berakhir";
    } else if (tanggalHabis != null && tanggalHabis.difference(now).inDays <= 3) {
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      HGap.xs,
                      if (item.hargaSewa != null)
                        Text(
                          "Rp ${item.hargaSewa.toString().replaceAllMapped(RegExp(r'(\d)(?=(\d{3})+$)'), (m) => '${m[1]}.')}",
